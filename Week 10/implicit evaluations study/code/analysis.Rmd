...
```
**Result:** Spearman's rank correlation was used to assess the relationship between mean self-reported evaluation and AMP score. This two-sided test was not significant (r(88) = -.13, p = 0.231). The results thus indicate a negative, but non-significant relationship between the two variables.
...
result_nonpar <- wilcox.test(mean_evaluation ~ gender, data = male_female_subset, 
...
```
**Results:** A Wilcoxon rank sum test (or Mann-Whitney U test) with continuity correction was performed to compare mean self-reported evaluations between men and women. Two-sided testing revealed a significant result (W = 622.5, p = 0.003), thus indicating that the two groups differ in their central tendency. More specifically, women's explicit self-reported evaluations were lower on average than men's.
