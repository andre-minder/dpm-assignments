...
Extract age and gender from the demographics raw file. All rows containing data from participants with a missing unique ID are removed (since they cannot be assigned to the other data, i.e. BFI and IAT, anyway). Missing values in the age and sex variable are substituted with "missing/error/others".
...
# Sanity check for the recoding (check correlation between items). First construct a correlation matrix and then check if all coefficients are positive.
...
- Check if all subscales, which have been completed by the participants, do not contain any missing values
- Calculate the mean of the completed subscales for each participant
- Check if the calculated mean lay between 1 and 6
...
data_bfi_exclude_a <- data_bfi_exclude_impossible_values |> 
  ...
  mutate(mean_a = ifelse(is.nan(bfi_mean_a), NA, bfi_mean_a)) |> 
  select(-n_na_a)
    
# Check for complete conscientiousness subscale and calculate its mean
...
# Check for complete extroversion subscale and calculate its mean
...
# Check for complete neuroticism subscale and calculate its mean
...
# Check for complete openness subscale and calculate its mean
