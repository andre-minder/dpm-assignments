...
data_processed |>
  count(name = "n") |>
  kable() |>
  add_header_above(header = c("Whole sample" = 1)) |> # print the results as a nice table with a header
...
data_processed_after_exclusions |>
  count(name = "n") |>
  kable() |>
  add_header_above(header = c("For analysis" = 1)) |> # print the results as a nice table with a header
  kable_classic(full_width = FALSE)
...
data_processed_after_exclusions |>
  ...
  kable() |>
  add_header_above(header = c("Age" = 2)) |> # print the results as a nice table with a header
  kable_classic(full_width = FALSE)
...
# calculate Cronbach's alpha for the agreeableness subscale
...
# calculate Cronbach's alpha for the conscientiousness subscale
...
# calculate Cronbach's alpha for the extroversion subscale
...
# calculate Cronbach's alpha for the neuroticism subscale
...
# calculate Cronbach's alpha for the openness subscale
...
This sections prints a series of histograms for all BFI subscale means and the D-score of the IAT.
...
ggplot(data_processed_after_exclusions, aes(x = bfi_mean_a)) +
  geom_histogram(binwidth = 1,
                 boundary = 0,
                 fill = viridis_pal(begin = 0.05, option = "plasma")(1), 
                 color = viridis_pal(begin = 0.10, option = "plasma")(1)) + 
...
# construct a correlation matrix (Pearson's r), round the results (2 digits), and print it as a nice table with a header
data_processed_after_exclusions |> 
  ...
  kable() |>
  add_header_above(header = c("Correlation Matrix IAT - BFI" = 7)) |> 
  kable_classic(full_width = FALSE)
...
# Filter out participants with missing sex values and conduct a two-sample t-test. Report the results with report()
data_processed_after_exclusions |> 
...
## Plots: IAT and BFI (Scatter Plots)

```{r}

# Scatter plot: Agreeableness
plot_iat_bfi_a <- data_processed_after_exclusions |> 
  mutate(extreme_values = ifelse(between(d_score, quantile(d_score, 0.1), quantile(d_score, 0.9)), "Normal Values", "Extreme Values")) |> # define the extreme values via quantiles (Extreme values are defined as the bottom and top 10% values)
  ...
  theme_classic() +
  theme(legend.position = c(0.9, 0.9)) + # change legend position
  guides(color = guide_legend("Outliers"), # rename the legend
         shape = guide_legend("Outliers")) +
  scale_shape_manual(values = c(17,16)) + # change the shapes
...
plot_iat_bfi_c <- data_processed_after_exclusions |> 
  ...
  xlab("Mean Conscientiousness") +
  ylab("D-score") +
  ggtitle("Scatter Plot: Conscientiousness and D-score") +
  theme_classic() +
...
# combining the scatter plots and adding title and caption
...
# print the combined plots (and suppress the warning/information messages)
...
# create a folder plots if it does not exist yet
